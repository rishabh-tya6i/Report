...
const Top20 = ({ top_algos = [] }) => {
  const data = top_algos
    .filter((item) => Array.isArray(item) && item.length === 2 && item[0]) // safe check
    .map(([algo_name, value]) => ({
      name: algo_name,
      value: isNaN(value) ? 0 : Number(value),
      fill: value < 0 ? '#EF4444' : '#16A34A', // red or green
    }));
  ...
  return (
    <div className="w-full max-w-lg h-auto p-4 m-3 bg-white dark:bg-gray-800 rounded-2xl shadow-md text-xs text-gray-800 dark:text-gray-200 font-medium">
      <h2 className="text-center text-sm font-semibold mb-4">Top 20 Algo Strategies</h2>
